...
function replaceBase(doc, section) {
	...
	base = (0, _core.qs)(head, "base");
            ...
            link.type = 'text/css';
            link.href = 'https://gitaseva.org/fonts/all_lang.css';
		...
		value: function adjustImages(contents) {
			...
			contents.addStylesheetRules({
				"img": {
					...
					"max-height": (this._layout.height ? 530 + "px" :  530 + "px") + "!important",
					"object-fit": "contain",
...
// 
            ...
            link1.type = 'text/css';
            link1.href = 'https://gitaseva.org/fonts1/all_lang.css';
			this.document.querySelector("head").appendChild(link1);
   ...
   */
		...
		if (url) {
			this.open(url).catch(function (error) {
				...
				_this.emit(_constants.EVENTS.BOOK.OPEN_FAILED, err, error);
			});
		}
	}
  ...
  */
   ...
   */
		...
		value: function unpack(toc) {
			...
			// 	item = toc[i];
			// 	if(item.subitems){
			// 		var itemLevel1 = item.subitems 
			// 		for(var j = 0; j < itemLevel1.length; j++) {
			// 			item = itemLevel1[j];
			// 			if(item.subitems){
			// 				var itemLevel2 = item.subitems 
			// 				for(var k = 0; k < itemLevel2.length; k++) {
			// 					item = itemLevel2[k];
			// 					this.tocByHref[item.href] = item.id-1;
			// 					this.tocById[item.id] = item.id-1;
			// 				}
			// 			}else{
			// 				this.tocByHref[item.href] = item.id-1;
			// 				this.tocById[item.id] = item.id-1;
		...
		value: function createTocItem(linkElement, id) {
			...
			for (var linkElm of tocLinkArry) {
				...
				if (linkElm.nodeName === 'li') {
					var tocLink = _core.qs(linkElm, 'a'),
						tocLinkData = {
							id: -1,
						...
						subItemElm = _core.qs(linkElm, 'ol');
   ...
   */
		...
		value: function parseNav(navHtml) {
			...
			return this.createTocItem(_core.qs(navElement, "ol"));
...
var Pane = exports.Pane = function () {
    ...
    _createClass(Pane, [{
        ...
        value: function addMark(mark) {
			...
			// if(this.marks[0].data.epubcfi.includes(mark.data.epubcfi)){

			// 	this.marks.push(mark);
			// }
			// if(this.marks[0].data.epubcfi == mark.data.epubcfi)
			this.marks.push(mark);
