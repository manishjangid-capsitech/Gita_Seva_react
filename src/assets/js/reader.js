...
EPUBJS.Reader = function (bookPath, _options) {
  ...
  if (this.book.settings.previousLocationCfi) {
    
    this?.displayed = this.rendition?.display(this.book.settings.previousLocationCfi);
  } else {
  ...
  book.ready.then(function () {
    ...
    this.rendition.on("keyup", reader.ReaderController.arrowKeys.bind(this));
...
EPUBJS.Reader.prototype.removeBookmark = function (cfi) {
  ...
  var b = document.getElementById(cfi)
...
EPUBJS.Reader.prototype.setBookKey = function (identifier) {
  if (!this.settings.bookKey) {
    ...
    this.settings.bookKey = "user:" + userid + ":" + window.location.host + ":" + identifier;
  }
...
EPUBJS.Reader.prototype.saveSettings = function () {
  if (this.book) {
    //this.settings.previousLocationCfi = this.rendition.currentLocation().end.cfi;
    this.settings.previousLocationCfi = this.rendition.location.end.cfi;
  }
...
EPUBJS.Reader.prototype.selectedRange = function (cfiRange) {
  ...
  var item = this.book.spine.get(cfiRange);
  var _request = this.book.load.bind(this)
...
EPUBJS.reader.BookmarksController = function () {
  ...
  divTitle.classList.add('booktitleClass');
  divTitle.textContent = "Bookmarks";
  bookmarks.appendChild(divTitle);
  ...
  var createBookmarkItem = function (cfi) {
    ...
    link.href = cfi;
    link.id = "a_" + cfi;
    link.style.color = localStorage.getItem('EpubColor') === 'black' ? 'white' : 'black';
    ...
    var elebkmrk = document.createElement('div');
    elebkmrk.id = cfi;
    var del_link = document.createElement("a");
    del_link.href = "javascript:void(0)";
    del_link.rel = cfi;
    del_link.classList.add('bookmark_del');
    del_link.classList.add('fa');
    del_link.classList.add('fa-trash');
    del_link.addEventListener("click", function (del) {
      reader.removeBookmark(del.target.rel);
      var $item = del.target.parentElement;
      $item.remove();

    }, false);
    elebkmrk.appendChild(link);
    elebkmrk.appendChild(del_link);
    listitem.appendChild(elebkmrk);
...
//     $main.addClass('black');
//     $sidebar.addClass('blackfilter');
//     $nightmode.removeClass('fa-moon-o');
//     $nightmode.addClass('fa-sun-o');
//     $nightmode[0].title = "Light Mode";
//     reader.rendition.themes.select("black");
//     localStorage.setItem('EpubColor', 'black');
//   }

//   var LightMode = function () {
//     $main.removeClass('black');
//     $sidebar.removeClass('blackfilter');
//     $nightmode.removeClass('fa-sun-o');
//     $nightmode.addClass('fa-moon-o');
//     $nightmode[0].title = "Dark Mode";
//     reader.rendition.themes.select("white");
//     localStorage.setItem('EpubColor', 'white');
//   }


//   $reset.on('click', function(){
//     if (reader.rendition.themes._current == "black") {
//       LightMode();
//     }
//   });
...
//     if (localStorage.getItem('EpubColor') == "black") {
//       DarkMode();
//     }
//     else {
//       LightMode();
...
//         if (fullscreen) {
//           $fullscreen
//             .addClass("compressScreen")
//             .removeClass("fullscreen");
//         } else {
//           $fullscreen
//             .addClass("fullscreen")
//             .removeClass("compressScreen");
//         }
//       });
//     }
//   }
...
//     if (els.length > 0) {
//       var e = els[0];
//       //$("#ChapterName").innerHTML = e.innerHTML;
//       var rel = e.rel;
//       if (rel == 1) {
//           document.getElementById('epubprogressbar').value = 0;
//           document.getElementById('pbvalue').innerHTML = "1% ";
//       } else {
//           document.getElementById('epubprogressbar').value = rel;
//           document.getElementById('pbvalue').innerHTML = Math.round((rel / document.getElementById('epubprogressbar').max) * 100) + "% ";
//       }
//     }
...
EPUBJS.reader.ControlsController = function (book) {
  ...
  var $fullscreen = $("#fullscreen"),
      ...
      $reset = $('#reset'),
      $tocSbView = $('#tocSbView'),
      $bookmarkSbView = $('#bookmarkSbView'),
      $highlightSbView = $('#highlightSbView'),
      $btnThemeWhite = $('#btnThemeWhite'),
      $btnThemeGrey = $('#btnThemeGrey'),
      $btnThemeIvory = $('#btnThemeIvory'),
      $btnThemeBlack = $('#btnThemeBlack'),
  ...
  $btnThemeBlack.on('click', function (e) {
      BlackMode();
  });

  $btnThemeGrey.on('click', function (e) {
      GreyMode();
  });

  $btnThemeIvory.on('click', function (e) {
      IvoryMode();
  });

  $reset.on('click', function (rendition) {
      var iframe = document.getElementsByTagName('iframe')
      var yu = document.getElementById(iframe[0].id);
      var body = yu.contentWindow.document.querySelector("body");
      body.style.fontSize = '20px';
      $("#my-range").val(20);
      $('#rangeVal').html('50%');
      localStorage.setItem('fontSize', '20px');
      //
      if ($('#divTheme:visible').length > 0) {
          $('#divTheme').slideToggle(100);
      }
      WhiteMode();
  })
  ...
  rendition.on('started', function () {
      ...
      else if (localStorage.getItem('CurrentTheme') == "grey") {
          GreyMode();
      }
      else if (localStorage.getItem('CurrentTheme') == "ivory") {
          IvoryMode();
      }
      else {
          WhiteMode();
  ...
  if (typeof screenfull !== 'undefined') {
      ...
      if (screenfull.raw) {
          document.addEventListener(screenfull.raw.fullscreenchange, function () {
              fullscreen = screenfull.isFullscreen;
              if (fullscreen) {
                  $fullscreen.addClass("compressScreen");
                  $fullscreen.removeClass("fullscreen");
              } else {
                  $fullscreen.addClass("fullscreen")
                  $fullscreen.removeClass("compressScreen");
              }
          });
      }
  }
  ...
  $bookmark.on("click", function () {
      ...
      if (bookmarked === -1) { //-- Add bookmark
          ...
          $bookmark.addClass("icon-bookmark-fill");
          //this.settings.bookmarks.push(cfi);

      } else { //-- Remove Bookmark
          reader.removeBookmark(cfi);
          var elem = document.getElementsByClassName(cfi);
          $(elem).empty();
          $bookmark.removeClass();
          $bookmark.addClass("icon-bookmark");
          //var bookmark = EPUBJS.rendition.isBookmarked(cfi);
          //this.settings.bookmarks.splice(bookmark, 1);
  ...
  rendition.on('relocated', function (location) {
      ...
      if ($("#SettingBar").is(":visible")) {
          $('#divTheme').css("display", "none");
          $('#SettingBar').slideToggle(100);
      }
...
EPUBJS.reader.SidebarController = function (book) {
  ...
  var changePanelTo = function (viewName) {
    ...
    $panels.find('.show_view').removeClass("active");
...
EPUBJS.reader.TocController = function (toc) {
  ...
  var generateTocItems = function (toc) {
     // Progressbar
      $('#epubprogressbar')[0].min = 0;
      $('#epubprogressbar')[0].max = toc.toc[toc.toc.length - 1].id;

    var divMain = document.getElementById("tocSbView");
    // var divTitle = document.createElement("div");
    //  divTitle.classList.add('titleClass');
    //  divTitle.textContent = "Index";
    //  divTitle.id ="indHead";
    var divList = document.createElement("ul");
    divList.classList.add('ePubSidebarMenu');
    // Chapter (Level 1)
    var count = 0;
    //if(!level) level = 1;
    toc.forEach(function (chapter) {
        var tocName = chapter.href.split('#')[1];
        var listitem = document.createElement("li"),
            link = document.createElement("a");
        //  
        listitem.id = 'td_' + tocName;
        //listitem.classList.add('list_item');
        link.textContent = chapter.label;
        link.href = chapter.href;
        link.id = tocName;
        link.rel = chapter.id + '#' + chapter.label;
        link.classList.add('toc_link');
        if (chapter.subitems && chapter.subitems.length > 0) {
            var eletogg = document.createElement('div');
            eletogg.classList.add('sidebarToggleLevel1');
            var ele_i = document.createElement('i');
            ele_i.classList.add('icon');
            ele_i.classList.add('icon-plus');
            var ele_clear = document.createElement('div');
            ele_clear.classList.add("clear");
            eletogg.appendChild(ele_i);
            eletogg.appendChild(ele_clear);
            listitem.appendChild(eletogg);
        }
        listitem.appendChild(link);
        ...
        count = 0
        if (chapter.subitems && chapter.subitems.length > 0) {
            var container_level_2 = document.createElement("ul");
            container_level_2.classList.add('menuLevel1');
            container_level_2.classList.add('dislayhide');
            // level++;
            // subitems = generateTocItems(chapter.subitems, level);
            //if (count == 0 ) {
            //    link.rel = link.rel + ' / ' + chapter.subitems[0].label;
            //}
            //count = 1

            chapter.subitems.forEach(function (chapter_level_2) {
                tocName = chapter_level_2.href.split('#')[1];
                var listitem_level_2 = document.createElement("li");
                var link_level_2 = document.createElement("a");
                listitem_level_2.id = 'td_' + tocName;
                //listitem_level_2.classList.add('list_items');
                link_level_2.textContent = chapter_level_2.label;
                link_level_2.href = chapter_level_2.href;
                link_level_2.id = tocName;
                link_level_2.rel = chapter_level_2.id + '#' + chapter.label +  ' / ' + chapter_level_2.label;
                link_level_2.classList.add('toc_link');
                listitem_level_2.appendChild(link_level_2);
                container_level_2.appendChild(listitem_level_2);

                if (chapter_level_2.subitems && chapter_level_2.subitems.length > 0) {
                    var eletogg = document.createElement('div');
                    eletogg.classList.add('sidebarToggleLevel2');
                    var ele_i = document.createElement('i');
                    ele_i.classList.add('icon');
                    ele_i.classList.add('icon-plus');
                    var ele_clear = document.createElement('div');
                    ...
                    eletogg.appendChild(ele_i);
                    eletogg.appendChild(ele_clear);
                    listitem_level_2.appendChild(eletogg);
                }
                ...
                if (chapter_level_2.subitems && chapter_level_2.subitems.length > 0) {
                    ...
                    //if (count == 0) {
                    //    listitem_level_2.rel = listitem_level_2.rel + ' / ' + chapter_level_2.subitems[0].label;
                    //}
                    //count = 1

                    var container_level_3 = document.createElement("ul");
                    container_level_3.classList.add('menuLevel2');
                    container_level_3.classList.add('dislayhide');
                    chapter_level_2.subitems.forEach(function (chapter_level_3) {
                        tocName = chapter_level_3.href.split('#')[1];
                        var listitem_level_3 = document.createElement("li");
                        var link_level_3 = document.createElement("a");
                        listitem_level_3.id = 'td_' + tocName;
                        link_level_3.textContent = chapter_level_3.label;
                        link_level_3.href = chapter_level_3.href;
                        link_level_3.id = chapter_level_3.href; //tocName;
                        link_level_3.classList.add('toc_link');
                        link_level_3.rel = chapter_level_3.id + '#' + chapter.label + ' / ' + chapter_level_2.label + ' / ' + chapter_level_3.label;
                        listitem_level_3.appendChild(link_level_3);
                        container_level_3.appendChild(listitem_level_3);
                        listitem_level_2.appendChild(container_level_3)
                    })
                }
                listitem.appendChild(container_level_2)
            })
        }
...
rendition.on('renderered', chapterChange);
  ...
  // $list.find(".openChapter").on("click", function (event) {
  ...
  $('#epubprogressbar').on("change", function (e) {
    var ind = $('#epubprogressbar').val();
    var url = $(".toc_link")[ind].getAttribute('href');
    rendition.display(url);
  })


  $(".toc_link").click(function () {
    $(".toc_link").removeClass('activesidebarmenu');
    $(this).addClass('activesidebarmenu')
  })

  $(".sidebarToggleLevel1").click(function () {
    $(".menuLevel1", this.parentElement).slideToggle(300), $("i", this).toggleClass("icon-minus icon-plus")
  })

  $(".sidebarToggleLevel2").click(function () {
    $(".menuLevel2", this.parentElement).slideToggle(300), $("i", this).toggleClass("icon-minus icon-plus")
  })
